...
var SpendRequestSchema = exports_external.object({
  borrowAsset: exports_external.string().optional(),
  borrowAmount: exports_external.union([exports_external.string(), exports_external.number()]),
  depositAmount: exports_external.union([exports_external.string(), exports_external.number()]).optional(),
  payee: exports_external.string(),
  planNonce: exports_external.union([exports_external.string(), exports_external.number()]).optional()
});
function toBigInt(v) {
  if (typeof v === "number") {
...
function decodeSpendRequest(payload) {
...
function encodePlanReport(plan) {
  return encodeAbiParameters(parseAbiParameters("address borrowAsset, uint256 borrowAmount, address payee, uint256 planExpiresAt, uint256 planNonce"), [plan.borrowAsset, plan.borrowAmount, plan.payee, plan.planExpiresAt, plan.planNonce]);
}
var httpTrigger = new HTTPCapability().trigger({
  authorizedKeys: []
});
var httpClient = new ClientCapability2;
function initWorkflow(cfg) {
  validateConfig(cfg);
  ...
  if (!network248)
    throw new Error("Network not found for config.chainSelectorName");
  const evmClient = new ClientCapability(network248.chainSelector.selector);
  return [
    handler(httpTrigger, async (runtime2, payload) => {
      ...
      if (planBorrowAmount <= 0n)
        throw new Error("plan.borrowAmount must be > 0");
      ...
      const writeRes = evmClient.writeReport(runtime2, {
      ...
      runtime2.log(`writeReport txStatus=${writeRes.txStatus} receiver=${cfg.receiverAddress}`);
      return {
        statusCode: 200,
...
async function main() {
  const runner = await Runner.newRunner({ configSchema: ConfigSchema3 });
